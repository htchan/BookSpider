...
backup_dir_location = '/backup/' + str(datetime.datetime.now())[:10]
...
database_path = [ 
    ...
    if os.path.isfile(os.path.join(database_dir_location, filename)) and filename.endswith('.db')
]

for database_location in database_path:
    db = sqlite3.connect(os.path.join(database_dir_location, database_location))
    f = open(os.path.join(backup_dir_location, database_location).replace('.db', '.sql'), 'w', encoding='utf8')
    f.write('\n'.join(db.iterdump()))
    f.close()
    db.close()