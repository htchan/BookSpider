...
func TestBook_baseURL(t *testing.T) {
	...
	con := config.BookConfig{
		URLConfig: config.URLConfig{Base: "http://some.book.site/base/%v"},
	}
...
func TestBook_fetchInfo(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		tempCon.URLConfig.Base = server.URL + "/no_updated/%v"
	...
	t.Run("zero length", func(t *testing.T) {
		...
		tempCon.URLConfig.Base = server.URL + "/zero_length/%v"
		book := Book{BookConfig: &tempCon, CircuitBreakerClient: &client}
		startFetch := time.Now()
		title, writer, typeStr, date, chapterStr, err := book.fetchInfo()
	...
	t.Run("400", func(t *testing.T) {
		...
		tempCon.URLConfig.Base = server.URL + "/400/%v"
		book := Book{BookConfig: &tempCon, CircuitBreakerClient: &client}
		startFetch := time.Now()
		title, writer, typeStr, date, chapterStr, err := book.fetchInfo()
	...
	t.Run("503", func(t *testing.T) {
		...
		tempCon.URLConfig.Base = server.URL + "/503/%v"
		book := Book{BookConfig: &tempCon, CircuitBreakerClient: &client}
		startFetch := time.Now()
		title, writer, typeStr, date, chapterStr, err := book.fetchInfo()
	...
	t.Run("missing date", func(t *testing.T) {
		...
		tempCon.URLConfig.Base = server.URL + "/missing_date/%v"
		book := Book{BookConfig: &tempCon, CircuitBreakerClient: &client}
		title, writer, typeStr, date, chapterStr, err := book.fetchInfo()
...
func TestBook_Update(t *testing.T) {
	...
	t.Run("fetch existing book from error", func(t *testing.T) {
		tempCon := con
		t.Parallel()
		...
		if !result || err != nil ||
			book.Title != "title" || book.Name != "writer" ||
			book.Type != "type" || book.UpdateDate != "date" ||
			book.UpdateChapter != "chapter" || book.Status != model.InProgress {
	...
	t.Run("fetch new book from existing book", func(t *testing.T) {
		tempCon := con
		t.Parallel()

		tempCon.URLConfig.Base = server.URL + "/updated/%v"
		book := Book{
			BookModel: model.BookModel{
				Title: "title", Type: "type", UpdateDate: "date",
				UpdateChapter: "chapter", Status: model.End,
		...
		result, err := book.Update()
		if !result || err != nil || book.HashCode == 0 ||
	...
	t.Run("fetch error from error book", func(t *testing.T) {
		tempCon := con
		t.Parallel()
	...
	t.Run("fetch error from existing book", func(t *testing.T) {
		tempCon := con
		t.Parallel()
		...
		result, err := book.Update()
		if err == nil || err.Error() != "zero length" || book.Title != "title" || book.Name != "writer" ||
			book.Type != "type" || book.UpdateDate != "date" || book.UpdateChapter != "chapter" {
			t.Errorf("book model: %v", book.BookModel)
			t.Errorf("writer model: %v", book.WriterModel)
			t.Errorf("error model: %v", book.ErrorModel)
			t.Errorf("update return result: %v, err: %v", result, err)
