...
func parseBook(body string) (*client.BookInfo, error) {
	...
	// parse title
	title := client.GetGoqueryContentWithoutChildren(doc.Find(bookTitleGoquerySelector))
	if title == "" {
		parseErr = errors.Join(parseErr, client.ErrBookTitleNotFound)
	}

	// parse writer
	writer := client.GetGoqueryContentWithoutChildren(doc.Find(bookWriterGoquerySelector))
	writer = strings.ReplaceAll(writer, "小说作者：", "")
	if writer == "" {
		parseErr = errors.Join(parseErr, client.ErrBookWriterNotFound)
	}

	// parse type
	bookType := client.GetGoqueryContentWithoutChildren(doc.Find(bookTypeGoquerySelector))
	if bookType == "" {
...
func isAvailable(body string) bool {
	return strings.Contains(body, "求书网")
}
