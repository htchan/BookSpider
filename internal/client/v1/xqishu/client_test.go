...
func TestNewClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			xqishuCli, ok := cli.(*xqishuClient)
			assert.True(t, ok)
			assert.NotNil(t, xqishuCli.cli)
			assert.NotNil(t, xqishuCli.decoder)

			testStr := "test string"
			decoded, err := xqishuCli.decoder.Decode(testStr)
...
func TestClient_GetBookChapterList(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: client.ChapterEntryList{
				{URL: serv.URL + "/chapter url 1", Title: "chapter name 1"},
				{URL: serv.URL + "/chapter url 2", Title: "chapter name 2"},
				{URL: serv.URL + "/chapter url 3", Title: "chapter name 3"},
				{URL: serv.URL + "/chapter url 4", Title: "chapter name 4"},
			},
			wantErr: "",
		},
		{
