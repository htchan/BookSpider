...
func AddAPIRoutes(router chi.Router, conf *config.BackendConfig, readDataServices service.ReadDataService) {
	router.Route(conf.APIRoutePrefix, func(router chi.Router) {
		...
		router.Get("/info", GeneralInfoAPIHandler(conf.AvailableSites, readDataServices))
